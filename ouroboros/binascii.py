...
# https://bitbucket.org/pypy/pypy/src/f2bf94943a41/lib_pypy/binascii.py

# TODO find out why brython sets the charset and to what?
CHARSET = 'utf-8'
...
def a2b_uu(s):
    ...
    length = (maybe_ord(s[0]) - 0x20) % 64

    def quadruplets_gen(s):
        while s:
            try:
                yield maybe_ord(s[0]), maybe_ord(s[1]), maybe_ord(s[2]), maybe_ord(s[3])
            except IndexError:
                s += '   '
                yield maybe_ord(s[0]), maybe_ord(s[1]), maybe_ord(s[2]), maybe_ord(s[3])
                return
...
def b2a_uu(s):
    ...
    def triples_gen(s):
        while s:
            try:
                yield maybe_ord(s[0]), maybe_ord(s[1]), maybe_ord(s[2])
            except IndexError:
                s += '\0\0'
                yield maybe_ord(s[0]), maybe_ord(s[1]), maybe_ord(s[2])
                return
    ...
    return chr(maybe_ord(' ') + (length & 0o77)) + ''.join(result) + '\n'
...
def a2b_base64(s):
    ...
    return bytes(''.join([chr(i) for i in res]), CHARSET)
...
def a2b_qp(s, header=False):
    ...
    while inp < len(s):
        if s[inp] == '=':
            ...
            odata.append(s[inp])
            inp += 1
    return bytes(''.join(odata), CHARSET)
    ...
    while inp < len(data):
        ...
        if (c > '~' or
            ...
            if linelen >= MAXLINESIZE:
                ...
                odata.append('\n')
                linelen = 3
            odata.append('=' + two_hex_digits(maybe_ord(c)))
            inp += 1
        else:
            if (istext and
...
def a2b_hqx(s):
    ...
    #return (bytes(''.join(result), CHARSET), done)
    ...
    def triples_gen(s):
        while s:
            try:
                ...
                yield tuple([maybe_ord(c) for c in s])
            s = s[3:]
...
def crc_hqx(s, crc):
    for c in s:
        crc = ((crc << 8) & 0xff00) ^ crctab_hqx[((crc >> 8) & 0xff) ^ maybe_ord(c)]
...
def crc32(s, crc=0):
    ...
    for c in s:
        if type(c) == int:
            ...
            crc = crc_32_tab[(crc ^ int(maybe_ord(c))) & 0xff] ^ (crc >> 8)

        #crc = crc_32_tab[(crc ^ long(maybe_ord(c))) & 0xffL] ^ (crc >> 8)
...
def b2a_hex(s):
    result = []
    for char in s:
        c = (maybe_ord(char) >> 4) & 0xf
        if c > 9:
            c = c + maybe_ord('a') - 10
        else:
            c = c + maybe_ord('0')
        result.append(chr(c))
        c = maybe_ord(char) & 0xf
        if c > 9:
            c = c + maybe_ord('a') - 10
        else:
            c = c + maybe_ord('0')
...
def a2b_hex(t):
    ...
    def pairs_gen(s):
        while s:
            try:
                yield table_hex[maybe_ord(s[0])], table_hex[maybe_ord(s[1])]
            except IndexError:
                if len(s):
    ...
    return bytes(''.join(result), CHARSET)
